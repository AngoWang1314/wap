<template>
	<view
		class="uni-grid"
		:class="{
			'uni-grid-no-border':!showBorder,
			'uni-grid-no-out-border':showBorder && !showOutBorder
		}">
		<view
			...
			<view
				...
				v-for="(item,index) in items"
			 	:key="index"
				:class="[
					index == columnNum ? 'uni-grid-item-last' : '',
					'uni-grid-item-' + type,
					i === 0 ? 'mb-9': ''
				]"
				@click="onClick(i,index)"
				:style="{
					visibility: item.seize ? 'hidden' : 'inherit'
				}">
				<view
					...
					<text class="uni-grid-item-text"
						:style="{color: item.textColor}">{{item.text}}</text>
				</view>
			</view>
		</view>
	</view>
